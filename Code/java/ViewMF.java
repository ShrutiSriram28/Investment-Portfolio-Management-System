...
public class ViewMF extends HttpServlet {  
    ...
    throws IOException, ServletException{  
        ...
        try {
            ...
            String sql = "SELECT * FROM purchased_mf WHERE username = '" + status + "'";
            ...
            "<head>"+
                ...
                "<title>Display Mutual Funds</title>"+
            ...
            "<body>"+
                ...
                "<div id=\"side_panel\" class=\"sidenav\">"+
                    "<div style=\"display:flex; flex-direction: column; justify-content:space-around; align-items: center; width: 100%; height:85%;\">"+
                        ...
                        "<form id=\"MFs_form\" action=\"http://localhost:7071/Miniproject/java/ViewStocks\" method=\"get\" style=\"display:flex; flex-direction: row; justify-content:space-evenly; align-items: center; width: 100%;\">"+
                ...
                "<p style=\"color:rgb(80, 80, 255); font-size: 3em; background-color: white; position:fixed; top:10vh; width: 70%;text-align: center;\"><b>PURCHASED MUTUAL FUNDS</b></p><br><br><br><br><br>");
            ...
            while(rs.next())
            {
                ...
                float units = Float.parseFloat(rs.getString("units"));
                float purchase_nav = Float.parseFloat(rs.getString("purchase_nav"));
                float nav = Float.parseFloat(rs.getString("nav"));
                float gain = (nav * units - purchase_nav * units);

                unrealised_gain += (nav * units - purchase_nav * units);
                ...
                out.print("<p style=\"display: inline\">Purchase nav: " + rs.getString("purchase_nav") + "</p><br>");
                out.print("<form action=\"http://localhost:7071/Miniproject/java/SellMF\" style=\"display: inline; float: right;\">"+
                    ...
                    "<input type=\"hidden\" id=\"purchase_nav_hidden" + count + "\" name=\"purchase_nav\" value="+ rs.getString("purchase_nav") +">"+
                    "<input type=\"hidden\" id=\"nav_hidden" + count + "\" name=\"nav\" value="+ rs.getString("nav") +">"+
                    "<input type=\"hidden\" id=\"amount_hidden" + count + "\" name=\"amount\" value="+ rs.getString("amount") +">"+
                    "<input type=\"hidden\" id=\"units_hidden" + count + "\" name=\"units\" value="+ rs.getString("units") +">"+
                ...
                "</form><br>");
                out.print("<p>Current nav: " + rs.getString("nav") + "</p><br>");
                out.print("<p style=\"display: inline\">amount: " + rs.getString("amount") + "</p><br>");
                out.print("<form action=\"http://localhost:7071/Miniproject/java/DeleteMF\" style=\"display: inline; float: right;\">"+
                    "<input type=\"hidden\" id=\"status" + count + "\" name=\"status\" value="+status+">"+
                    "<input type=\"hidden\" id=\"scheme_hidden" + count + "\" name=\"scheme\" value="+rs.getString("scheme")+">"+
                    "<input type=\"hidden\" id=\"purchase_nav_hidden" + count + "\" name=\"purchase_nav\" value="+ rs.getString("purchase_nav") +">"+
                    "<input type=\"hidden\" id=\"nav_hidden" + count + "\" name=\"nav\" value="+ rs.getString("nav") +">"+
                ...
                "</form><br>");
                out.print("<p>Units: " + rs.getString("units") + "</p><br></div><br><br><br>");
                flag = 1;
            }
            if(flag == 0)
            {
            ...
            out.println("<form action=\"http://localhost:7071/Miniproject/java/AddMF\" method=\"get\">" +
                    "<input type=\"hidden\" id=\"status\" name=\"status\" value='" + status + "'>"+ 
            ...
            if(rs1.next())
            {
                float realised_gain = rs1.getFloat("MF");
